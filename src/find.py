...
def solve_milp(original, u, klass,c, nb_classes, weights=None):
    ...
    for c in range(nb_classes): #constraint klass is the original chosen class
        if c!=klass:
            constraints.append([[y[(t,v)] for t in range(original_nb_trees) for v in original[t]],
    ...
    for t in range(len(original)): #constraint 6
        constraints.append([y[(t,original[t][0])],[1.]])
        senses.append('E')
        rhs.append(1.)
    for t in range(len(original)): #constraint 7
        for v in original[t]:
            if not v.is_leaf:
                constraints.append([[y[(t,v)],y[(t,v.children[0])],y[(t,v.children[1])]],[1.,-1.,-1.]])
                senses.append('E')
                rhs.append(0.)
    for t in range(len(original)): #constraint 8
        for d in range(original[t][0].height):
            constraints.append([[lbda[(t,d)]]+[y[(t,v.children[0])] for v in original[t] if ((not v.is_leaf) and v.depth==d)],[1.]+[-1. for v in original[t] if ((not v.is_leaf) and v.depth==d)]])
            senses.append('G')
            rhs.append(0.)
    ...
    for t in range(len(original)):
        for v in original[t][0].leaves:
            objective.append((y[(t,v)],u[t]*weights[t]*(p[(t,v,klass)]-p[(t,v,c)])))
